...
if [[ -z "${PATHSPACE_LEGACY_WIDGET_BUILDERS_REPORT:-}" ]]; then
  export PATHSPACE_LEGACY_WIDGET_BUILDERS_REPORT="$BUILD_DIR/legacy_builders_usage.jsonl"
fi
...
if [[ -d "$BUILD_DIR/tests" ]]; then
  ...
  if [[ "$TEST" -eq 1 ]]; then
    ...
    if [[ -n "${PATHSPACE_CAPTURE_FONT_DIAGNOSTICS:-}" ]]; then
      TEST_ENV_FLAGS+=("--env" "PATHSPACE_CAPTURE_FONT_DIAGNOSTICS=${PATHSPACE_CAPTURE_FONT_DIAGNOSTICS}")
    elif [[ "$LOOP" -gt 0 ]]; then
