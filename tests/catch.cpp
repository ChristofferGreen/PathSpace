...
namespace Catch
{
    namespace TestCaseTracking
    {
        ...
        TrackerBase::TrackerBase(NameAndLocation const &nameAndLocation, TrackerContext &ctx, ITracker *parent) : ITracker(nameAndLocation, parent),
                                                                                                                  m_ctx(ctx)
        {
        }
        ...
        void printTotals(std::ostream &out, const Totals &totals, ColourImpl *colourImpl)
        {
            ...
            else if (totals.testCases.failed == totals.testCases.total())
            {
                ...
                const StringRef qualify_assertions_failed =
                    totals.assertions.failed == totals.assertions.total() ? bothOrAll(totals.assertions.failed) : StringRef{};
                out << "Failed " << bothOrAll(totals.testCases.failed)
                    << pluralise(totals.testCases.failed, "test case"_sr) << ", "
                                                                             "failed "
                    << qualify_assertions_failed << pluralise(totals.assertions.failed, "assertion"_sr) << '.';
            }
            ...
            {
                out << colourImpl->guardColour(Colour::ResultError) << "Failed " << pluralise(totals.testCases.failed, "test case"_sr) << ", "
                                                                                                                                          "failed "
                    << pluralise(totals.assertions.failed, "assertion"_sr) << '.';
            }
            else
            {
                out << colourImpl->guardColour(Colour::ResultSuccess) << "Passed " << bothOrAll(totals.testCases.passed)
                    << pluralise(totals.testCases.passed, "test case"_sr) << " with " << pluralise(totals.assertions.passed, "assertion"_sr) << '.';
            }
        }
