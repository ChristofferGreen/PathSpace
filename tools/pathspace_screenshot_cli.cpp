...
std::optional<CliArgs> parse_cli(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        auto require_value = [&](std::string_view flag) -> std::optional<std::string> {
            if (i + 1 >= argc) {
                std::cerr << "pathspace_screenshot_cli: " << flag << " expects a value\n";
                return std::nullopt;
            }
        ...
        if (token == "--baseline") {
            ...
            args.baseline_override = std::filesystem::path{*value};
            continue;
        }
        if (token == "--screenshot-output") {
            auto value = require_value(token);
            if (!value) {
                return std::nullopt;
            }
            args.screenshot_output = std::filesystem::path{*value};
            continue;
        }
        if (token == "--diff-output") {
            auto value = require_value(token);
            if (!value) {
                return std::nullopt;
            }
            args.diff_output = std::filesystem::path{*value};
            continue;
        }
        if (token == "--metrics-output") {
            auto value = require_value(token);
            if (!value) {
                return std::nullopt;
            }
            args.metrics_output = std::filesystem::path{*value};
            continue;
        }
        if (token == "--tolerance") {
            auto value = require_value(token);
            ...
            args.tolerance_override = std::stod(*value);
            continue;
        }
