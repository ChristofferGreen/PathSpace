...
- Include the paint example screenshot card by default: expose `/diagnostics/ui/paint_example/screenshot_baseline/*` (or the JSON snapshots written by `--screenshot-metrics-json`) so the inspector dashboard highlights baseline mismatches before engineers open PNGs.
