...
- Local preview now lives at `http://localhost:8765/` via `scripts/paint_example_inspector_panel.py`. The helper provides both JSON (`/api/cards/paint-example`) and SSE (`/api/cards/paint-example/events`) endpoints by shelling into `pathspace_paint_screenshot_card --json`, emitting `card` / `card-error` events whenever the diagnostics tree changes. Phase 0 should adopt this exact payload + event contract inside the web adapter so the browser UI no longer depends on the Python shim once the backend routes land.
