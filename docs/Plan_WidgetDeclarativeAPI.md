...
1. **Examples**
   ...
   - ✅ (November 19, 2025) `examples/paint_example.cpp` gained a `--screenshot <path>` flag that runs headless, replays a scripted brush sequence via `PaintRuntime::HandleAction`, forces framebuffer capture, and writes a PNG with stb_image_write. This enables the “look → fix → look” visual diff loop for the declarative paint stack without opening a window—run the flag after tweaks to compare outputs side-by-side.
   ...
   - Remaining TODOs: move the legacy paint GPU staging smoke test to the declarative sample once the uploader lands, and add declarative paint parity/fuzz coverage (focus traversal, random stroke replay) under `tests/ui/test_DeclarativePaintSurface.cpp` + the WidgetEventTrellis fuzz harness.
2. **Tests**
