...
- ✅ **(November 26, 2025 — Highest Priority)** Loop stability work:
  - ✅ `PumpWindowWidgetsOnce` now drains a single window/view synchronously, records per-window/app counters under `/system/widgets/runtime/input/{windows,apps}/...`, and the input-task doctests call it directly instead of waiting for the global worker. UITests can watch those counters to prove their widgets were serviced even when `PATHSPACE_TEST_TIMEOUT=1`.
  - ✅ `PathSpaceExamples::ensure_declarative_scene_ready` accepts `scene_window_component_override` / `scene_view_override` / `ensure_scene_window_mirror`, so doctests can target the exact `/scenes/<scene>/structure/widgets/windows/<window>/views/<view>/widgets` branch they rely on. The lifecycle doctest now points it at the window it just created and no longer waits forever when the default mapping doesn’t match.
  - ✅ `tests/ui/test_PaintExampleNew.cpp` forces the window renderer into Software2D via the new `force_window_software_renderer` helper, so Metal-upload stress no longer blocks pointer subscription smoke tests.
  - ✅ Long-running UITests (`Scene dirty event wait-notify latency stays within budget`, `WidgetEventTrellis fuzzes declarative paint stroke ops`) read `PATHSPACE_TEST_TIMEOUT` and scale their waits/iteration counts unless `PATHSPACE_FULL_FUZZ=1`, keeping the 20 s harness headroom intact without losing the more exhaustive coverage when desired.
  - ✅ (November 26, 2025) Added a `force_scene_publish` option to `PathSpaceExamples::ensure_declarative_scene_ready` that retries `SceneLifecycle::ForcePublish` before waiting on revisions. UITests that only need runtime metrics now skip the legacy bucket/structure waits (which routinely exceeded the 20 s loop budget) and rely on the forced publish result instead. The first forced publish after a clean build still needed deterministic buckets, which the new manual pump (next bullet) addresses.
  - ✅ (November 26, 2025) Implemented `SceneLifecycle::PumpSceneOnce` plus `DeclarativeReadinessOptions::pump_scene_before_force_publish` / `scene_pump_options`. The manual pump runs on the lifecycle worker thread, registers any newly mounted widgets, optionally re-marks them dirty, processes pending `render/dirty` nodes, and updates `widgets_with_buckets` before each forced publish attempt. All declarative UITests now inherit the pump via `ensure_declarative_scene_ready`, so the first forced publish after a clean build succeeds without waiting for the background worker to catch up. `tests/ui/test_DeclarativeSceneLifecycle.cpp` (“Scene lifecycle manual pump synthesizes widget buckets”) covers the API directly.
  - ✅ (November 26, 2025) Added the loop-stability CI job (`loop-stability` in `.github/workflows/ci.yml`). It runs `./scripts/compile.sh --clean --test --loop=5 --release --per-test-timeout 20` with `PATHSPACE_TEST_TIMEOUT=1`, keeps PathSpaceUITests logs via `PATHSPACE_LOOP_KEEP_LOGS`, emits `loop_baseline.json`, and ingests the new manual-pump metric snapshots so we always capture a fresh baseline log plus per-window/app counters before merging declarative changes. The job uploads `build/test-logs/` (manifest, manual pump summary, artifacts) for dashboards.
  - ✅ (November 26, 2025) `ScreenshotService::Capture` now accepts `force_software` so the paint example can skip `Window::Present` entirely when headless Metal refuses hardware capture. The deterministic paint replay writes the PNG via the fallback hook, so loops keep producing artifacts even when Metal is unavailable.
  - ✅ (November 26, 2025) `pathspace_screenshot_cli` learned `--force-software`/`--allow-software-fallback` plus metrics validation. It now logs `hardware_capture`, exits non-zero when fallback happens without the guard, and surfaces the capture mode in `build/artifacts/paint_example/*_metrics.json` so dashboards can alert if software fallback becomes the norm.
  - ✅ (November 26, 2025) Added the `PATHSPACE_SCREENSHOT_FORCE_SOFTWARE=1` guard that loop-stability can set. Both the CLI and the thin Python wrapper honor it (and the guard propagates down to `examples/paint_example.cpp`), letting CI opt into the fallback while default runs still demand hardware captures.
...
3. **Removal**
   ...
   - ✅ (November 26, 2025) Introduced `include/pathspace/ui/PathTypes.hpp` and migrated the declarative runtime, screenshot helpers, examples, and doctest harnesses to the shared `SP::UI::{WindowPath,ScenePath}` aliases so they no longer depend on `SP::UI::Builders::*` for fundamental path handles. This confines the remaining builder usage to theme/style structs and render helpers ahead of the final removal step.
   - ✅ (November 27, 2025) Extracted every shared widget datatype into `include/pathspace/ui/WidgetSharedTypes.hpp` (paths, themes, stack enums, handler payloads, `DirtyRectHint`, `WidgetAction`, etc.) and taught declarative headers/tests to include it instead of `Builders.hpp` when they only need data. `Builders.hpp` now re-exports the header for compatibility, so downstream projects can migrate incrementally before the kill switch ships.
   - Delete legacy builders/samples after window; simplify remaining docs.
   - **Remaining removal steps (November 27, 2025):**
     1. ✅ (November 27, 2025) **Replace runtime helper usage.** Added `SP::UI::Declarative::{BuildPresentHandles,ResizePresentSurface,PresentWindowFrame,PresentFrameToLocalWindow}` and rewired `SP::App::RunUI`, the screenshot service, declarative helpers, paint/widgets/devices examples, and `tests/ui/test_PaintExampleNew.cpp` to rely on these wrappers instead of `Builders::App::BootstrapResult`, `Builders::Window::Present`, or `Builders::App::PresentToLocalWindow`. Declarative callers now keep renderer/surface/present plumbing entirely inside the declarative runtime, so the next steps can focus on theme/config migration without dragging the legacy builders back in.
     2. ✅ (November 27, 2025) **Migrate theme + config APIs.** `Builders::Config::Theme::*` now forward to the declarative-only helpers exposed via `SP::UI::Declarative::ThemeConfig::{SanitizeName,Resolve,Ensure,Load,SetActive,LoadActive}`. Declarative runtime, descriptor loaders, tests, and samples include the new header directly, so theme edits no longer require `Builders.hpp`. Docs (`WidgetDeclarativeAPI.md`, `Widget_Schema_Reference.md`, migration tracker) call out the new namespace, leaving the legacy wrappers as shims until the kill switch ships.
    3. **Retire builder-specific tests/benchmarks.** ✅ (November 27, 2025) Removed `tests/ui/test_Builders.cpp` and `tests/ui/test_AppBootstrap.cpp` now that the declarative UITests cover focus, bindings, paint, HTML, and presenter flows. `benchmarks/ui/widget_pipeline_benchmark.cpp` plus the `widget_pipeline` scenario in `scripts/perf_guardrail.py` now record declarative-only metrics (mutate throughput, bucket cost, paint GPU upload latency), and `docs/perf/performance_baseline.json` was refreshed to drop the legacy columns. CI no longer depends on legacy builder binaries.
     4. **Delete sources + refresh docs.** Once the tree compiles without `<pathspace/ui/Builders.hpp>`, remove the header/sources from `src/pathspace/ui/`, drop them from CMake, run the mandated `cmake --build …` + `./scripts/compile.sh --clean --test --loop=5 --release` validation, and archive the legacy docs under `docs/finished/` with a final status note in `docs/Memory.md`.
        - ⚠️ (November 27, 2025) **Current blockers:** declarative runtime, samples, and UITests still rely on `SP::UI::Builders` for reducers, renderer/surface provisioning, dirty-rect math, and text bucket assembly. The following sub-work streams must land before we can delete the files:
          1. ✅ (November 27, 2025) **Detail utilities extraction.** Added `include/pathspace/ui/declarative/Detail.hpp` (plus `DetailShared.hpp`) so declarative sources/tests consume `SP::UI::Declarative::Detail::*` wrappers for `read_optional`, `replace_single`, dirty-rect helpers, stack metadata, and text-field bucket synthesis. Only `src/pathspace/ui/declarative/Detail.cpp` still includes `BuildersDetail.hpp`, confining legacy references to a single bridge.
          2. ✅ (November 27, 2025) **Bootstrap/present parity.** Declarative runtime, readiness helpers, and screenshot tooling now resize/present entirely via `SP::UI::Declarative::{ResizePresentSurface,PresentWindowFrame}`. The new detail bridge exposes `prepare_surface_render_context`, render/diagnostics writers, dirty-rect submission, and present metrics so runtime code no longer calls `Builders::App::UpdateSurfaceSize` or `Builders::Window::Present`, yet still repopulates `windowTarget` hints, renderer settings, and framebuffer nodes exactly as the legacy helpers did.
         3. ✅ (November 27, 2025) **Reducer + text pipeline migration.** Declarative runtime now owns `SP::UI::Declarative::Reducers::{WidgetOpsQueue,ReducePending,PublishActions,ProcessPendingActions}` (`include/pathspace/ui/declarative/Reducers.hpp`, `src/pathspace/ui/declarative/Reducers.cpp`). `InputTask`, paint runtime, examples, and every declarative UITest consume the new helpers directly, while the legacy `SP::UI::Builders::Widgets::Reducers::*` wrappers simply forward after running the guard so remaining compatibility suites keep their coverage. Text synthesis moved to `SP::UI::Declarative::Text` (`include/pathspace/ui/declarative/Text.hpp`, `src/pathspace/ui/declarative/Text.cpp`); the legacy `TextBuilder.hpp` header re-exports the declarative types so builder call sites keep compiling without holding declarative code hostage. `Descriptor.cpp` now calls the declarative text builder when emitting label buckets, and doctests that only touch declarative widgets no longer need bespoke `ScopedAllow` overrides (the global reporter still auto-allows `tests/ui/test_TextBuilder.cpp` until that compatibility suite lands on the declarative types directly).
         4. **Kill-switch dry run.** After the three migrations above, enable `-DPATHSPACE_DISABLE_LEGACY_BUILDERS=ON` in CI/pre-push for at least one full loop (5×) to prove no production code includes the header. Only once the kill switch build is clean should we delete the legacy sources and follow the validation/doc archival steps in this bullet. ✅ (November 27, 2025) Landed `scripts/kill_switch_dry_run.sh`, a thin wrapper around `scripts/compile.sh` that automatically sets `-DPATHSPACE_DISABLE_LEGACY_BUILDERS=ON`, defaults to `build-kill-switch/`, and runs the mandated `--clean --release --loop=5 --per-test-timeout 20` suite so contributors can dry-run the guard on demand. **Update (November 27, 2025):** Added `include/pathspace/ui/BuildersShared.hpp`, a thin wrapper that defines `PATHSPACE_LEGACY_ALLOW_BUILDERS_HEADER` before including `Builders.hpp`. All production sources, benchmarks, tests, and tools now include the wrapper (or `WidgetSharedTypes.hpp`) instead of the legacy header, so the kill-switch build succeeds and any direct include continues to trip the compile-time error. Wire the dry-run script into `scripts/git-hooks/pre-push.local.sh` and the `loop-stability` CI job now that the tree builds cleanly with the flag enabled.
         - ✅ (November 27, 2025) Wired `scripts/kill_switch_dry_run.sh` into `scripts/git-hooks/pre-push.local.sh` and the `loop-stability` CI job so every pre-push run and the macOS loop harness exercise the kill switch automatically (dedicated `build-kill-switch[-loop]/` trees plus JSONL reports).
         - ✅ (November 28, 2025) `tests/ui/test_WidgetReducersFuzz.cpp` now runs entirely on the declarative runtime, so kill-switch builds pass without `SKIP_KILL_SWITCH_DRY_RUN=1`. The dry-run script remains in both guardrails to keep the flag honest whenever new builder references slip back in.
         5. ✅ (November 27, 2025) **Surface binding migration.** `SP::UI::Surface::SetScene` now performs the app-root validation + `/scene`/`/target` rewrites directly via `SP::UI::Declarative::Detail` helpers. Every example/test that previously invoked `SP::UI::Builders::Surface::SetScene` has been moved to the helper namespace, so the last widely used builder entry no longer leaks outside the legacy module. Parity coverage lives in `tests/ui/test_SceneHelpers.cpp` (“Surface::SetScene requires shared app root”), which now asserts on helper errors instead of comparing against the legacy implementation.
         6. ✅ (November 27, 2025) **Scene dirty helpers extracted.** `SP::UI::Declarative::SceneLifecycle::{MarkDirty,ClearDirty,ReadDirtyState,TakeDirtyEvent}` now implement the dirty queue/state contract directly, and the legacy `SP::UI::Builders::Scene::*` wrappers simply call through. Declarative runtime, examples, and docs no longer depend on the builder header for dirty markers, clearing another blocker for deleting the files.
        7. ✅ (November 28, 2025) **Surface/runtime type extraction.** Path surface/render helpers (`include/pathspace/ui/PathSurface{Software,Metal}.hpp`, `include/pathspace/ui/PathRenderer2D*.hpp`, `src/pathspace/ui/{PathSurface*,PathRenderer2D*}.cpp/mm`, `src/pathspace/ui/Helpers.hpp`) plus the HTML benchmark/example/test suites (`benchmarks/ui/path_renderer2d_benchmark.cpp`, `examples/{html_replay_example,pixel_noise_example}.cpp`, `tests/ui/test_{PathRenderer2D*,PathSurfaceSoftware,PathWindowView*,BackendAdapters*,RendererFaultInjection,HtmlReplay}.cpp`, `tests/tools/html_canvas_dump.cpp`) now include `<pathspace/ui/runtime/SurfaceTypes.hpp>` directly and reference `SP::UI::Runtime::{SurfaceDesc,SoftwareFramebuffer,PixelFormat,ColorSpace,Metal*}`. This removes their implicit dependency on `<pathspace/ui/SurfaceTypes.hpp>` / `Builders.hpp` for descriptor access.
        8. ✅ (November 28, 2025) **Renderer knob extraction.** Added `<pathspace/ui/runtime/RenderSettings.hpp>` so `RendererKind`, `RenderSettings`, and `DirtyRectHint` live under `SP::UI::Runtime`. PathRenderer2D, renderer helpers, declarative runtime, benchmarks, HTML/backend tests, and examples now include the runtime header directly and refer to `SP::UI::Runtime::{RendererKind,RenderSettings,DirtyRectHint}`. `Builders.hpp` and `WidgetSharedTypes.hpp` simply alias the runtime types for compatibility, clearing the last pre–kill-switch blocker.
4. **Post-mortem**
