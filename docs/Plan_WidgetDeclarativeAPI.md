...
### Remaining TODOs After Screenshot Fix
- ✅ (November 20, 2025) Screenshot automation now rides the GPU smoke harness: `scripts/check_paint_screenshot.py` runs `./build/paint_example --gpu-smoke --screenshot <tmp> --screenshot-require-present --screenshot-compare=docs/images/paint_example_baseline.png`, captures the real `Window::Present` framebuffer, and diffs it against the baseline (optionally writing `paint_example_screenshot_diff.png`). The helper enforces a `0.0015` mean absolute error threshold to account for the slider highlight’s sub-pixel jitter; diffs > threshold keep the artifact for triage. The new `PaintExampleScreenshot` CTest target fails the build if the framebuffer deviates beyond that tolerance or if the example falls back to the software renderer, so we finally guard the declarative paint UI visually instead of trusting the deterministic rasterizer.
- ✅ (November 20, 2025) `examples/paint_example --screenshot` now composites the deterministic software strokes into the captured framebuffer below the toolbar. The toolbar still proves `Window::Present` is alive, while the scripted canvas region always reflects the intended brush pattern even on hosts where `SceneLifecycle` still drops the `StrokeCommand` bucket. Set `PAINT_EXAMPLE_DEBUG=1` to dump lifecycle metrics/descriptor state when chasing regressions.
- ✅ (November 24, 2025) Documented the capture/wait workflow via `ensure_declarative_scene_ready`, updated onboarding + schema references, and switched every declarative sample (widgets examples, devices demo, declarative hello, paint examples) to call the helper before presenting so the readiness contract is enforced uniformly.
- ✅ (November 24, 2025) Eliminated the “stroke command references point buffer out of range” failure during screenshot force-publish: paint strokes now record a per-stroke `state/history/<id>/version` counter, writes increment the version after committing points, and descriptors retry until they read matching versions before building buckets. SceneLifecycle therefore only caches structurally valid buckets, the paint screenshot overlay no longer masks broken publishes, and the TODO is closed.
- ⚠️ (November 26, 2025) `./scripts/compile.sh --clean --test --loop=5 --release` still flakes under the `PATHSPACE_TEST_TIMEOUT=1` harness. Loop 1 timed out while `tests/ui/test_DeclarativeRuntime.cpp` (“Declarative input task drains widget ops”) waited for `/widgets/<id>/ops/actions/inbox/queue` to populate, and the same run received a SIGTERM in `tests/ui/test_DeclarativeSceneLifecycle.cpp` (“Scene lifecycle publishes scene snapshots and tracks metrics”). Logs: `build/test-logs/PathSpaceUITests_loop1of5_20251126-091725.log`. **Action:** Harden the declarative runtime tests by (a) reusing `PathSpaceExamples::ensure_declarative_scene_ready`, (b) waiting for `/system/widgets/runtime/input/metrics/widgets_processed_total` to advance before asserting on actions, and (c) bumping the lifecycle worker wait budget for the test-only scene so at least one publish completes before the timeout trips. Keep this entry open until the loop runs reliably again.
