...
3. **Removal**
   ...
   - **Remaining removal steps (November 27, 2025):**
     ...
     2. **Migrate theme + config APIs.** Relocate `Builders::Config::Theme::*` functionality into `SP::UI::Declarative::Theme`, update docs (`WidgetDeclarativeAPI.md`, onboarding guides, schema references) and samples to consume the declarative entry points only, and add regression tests that cover theme creation/edits without bringing in `Builders.hpp`.
