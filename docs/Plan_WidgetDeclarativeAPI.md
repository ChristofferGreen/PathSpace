...
3. **Removal**
   ...
   - **Remaining removal steps (November 27, 2025):**
     1. **Replace runtime helper usage.** Teach `SP::App::RunUI`, the screenshot service, and headless helpers to expose the renderer/surface/present plumbing currently pulled from `Builders::App::BootstrapResult`, then update `examples/declarative_example_shared.hpp`, paint/screenshots, widgets/devices demos, and tests to call the new declarative helpers instead of `Builders::Window::Present`, `App::UpdateSurfaceSize`, or `App::PresentToLocalWindow`.
