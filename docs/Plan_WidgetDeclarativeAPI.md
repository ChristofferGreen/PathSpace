...
- ⚠️ **(November 26, 2025 — Highest Priority)** Loop stability work:
  ...
  - 🔜 Re-run the full `./scripts/compile.sh --clean --test --loop=5 --release` harness with `PATHSPACE_TEST_TIMEOUT=1` in CI to capture a fresh baseline log, then decide whether the manual pump counters should also feed the shared dashboards (today they only unblock doctests).
