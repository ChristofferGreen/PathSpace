...
- Automate a regression that runs `examples/paint_example --screenshot` once the capture path is stable in CI (hook it into the existing GPU smoke harness so we diff the real framebuffer instead of the fake rasterizer output).
- Document the new capture/wait workflow in `docs/Widget_Schema_Reference.md` + onboarding so other declarative samples follow the same revision gate and `capture_framebuffer` contract.
