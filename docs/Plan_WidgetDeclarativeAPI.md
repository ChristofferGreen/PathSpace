...
5. **UI readiness guard (November 20, 2025)** — Added `wait_for_widget_buckets` + `wait_for_scene_widgets` to `examples/paint_example.cpp`. The helpers count `window_view/widgets/*`, poll `/runtime/lifecycle/metrics/widgets_with_buckets`, and block until `/scenes/<scene>/structure/widgets/windows/<window>/views/<view>/widgets` mirrors the expected entries, so both the interactive run loop and the screenshot path only start once `SceneLifecycle` has published every widget bucket. This removes the last “empty window” flashes during startup.
...
- Document the capture/wait workflow (metrics gate + scene structure check) across the other declarative samples and onboarding so every demo uses the same readiness contract before presenting.
- Open TODO: `SceneLifecycle` occasionally reports “stroke command references point buffer out of range” when the second present happens during the screenshot flow. The overlay keeps the PNG stable, but we still need to fix the underlying bucket validation so we can remove the workaround.
