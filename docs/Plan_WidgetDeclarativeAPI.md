...
3. **Removal**
   ...
   - **Remaining removal steps (November 27, 2025):**
     ...
     3. **Retire builder-specific tests/benchmarks.** Re-write the remaining `tests/ui/test_Builders.cpp`, `test_AppBootstrap.cpp`, and perf harness comparisons to rely solely on the declarative runtime (or delete obsolete scenarios) so CI coverage does not depend on the legacy entry points.
