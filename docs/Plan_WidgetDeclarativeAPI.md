...
- ⚠️ **(November 26, 2025 — Highest Priority)** Loop stability work:
  ...
  - ⚠️ (November 26, 2025) `PaintExampleScreenshot` still flakes under the PATHSPACE_TEST_TIMEOUT=1 loop because `Window::Present` refuses hardware capture on headless runners (`unknown_error:hardware capture required but Window::Present failed`). Follow-up plan: (1) teach `pathspace_screenshot_cli`/ScreenshotService to fall back to the deterministic software capture when the LocalWindow bridge declines hardware capture, (2) add a `PATHSPACE_SCREENSHOT_FORCE_SOFTWARE=1` guard that the loop-stability job can set until Metal capture is reliable again, and (3) update the screenshot tests to record which code-path ran (hardware vs. CPU fallback) so guardrails can alert when the fallback becomes permanent.
