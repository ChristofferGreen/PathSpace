...
- ✅ (November 7, 2025) Execution payloads remain opt-out: `HistoryOptions::executionOptOutPrefixes` lets callers register undo-exempt subtrees, snapshot capture skips those branches, and `_history/executionOptOut/*` telemetry tracks opt-out hits so integrators can confirm guardrails.
...
7. ✅ (November 7, 2025) Lock execution payload handling to opt-out prefixes and document the guardrails (`docs/Plan_PathSpace_UndoHistory.md`, `_history/executionOptOut/*` telemetry, unit coverage).
8. Run the Alpaca vs. binary metadata bake-off for persistence, record the findings in `docs/AI_Architecture.md`, and wire the winning format into stack metadata.
