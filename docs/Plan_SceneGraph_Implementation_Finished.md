...
> **Completion (October 29, 2025):** Manifest-driven font lookup resolves fallback chains via `FontManager::resolve_font`, surfaces active atlas revisions, and updates widget examples/tests to hydrate typography directly from manifest metadata; doctests cover both happy-path manifests and malformed fallback definitions.
