...
| --- | --- | --- | --- | --- | --- |
| **PathSpace Inspector (web)** | `docs/Plan_PathSpace_Inspector.md` | **In progress:** `InspectorSnapshot` + `InspectorHttpServer` now serve `/inspector/tree`, `/inspector/node`, and paint screenshot cards from live declarative paths; browser UI still pending. | Embed `InspectorHttpServer` inside apps (or use `pathspace_inspector_server` for demos) so inspector panels consume declarative widget state without legacy builders. Next milestone: hook the web panel up to the new endpoints and retire `scripts/paint_example_inspector_panel.py`. | - Monitor `/_system/diagnostics/legacy_widget_builders/status/usage_total` per `SP::UI::Builders::*` entry.<br>- Exercise `/inspector/tree` + `/inspector/cards/paint-example` in CI to ensure declarative scenes stay readable.<br>- Track open issues in `docs/Plan_PathSpace_Inspector.md#Next Actions`. | 2025‑11‑26 |
