...
- Immediate next steps:
  ...
  - Capture and track FPS deltas for representative canvases. (2025-10-19: `path_renderer2d_benchmark` on the reference Mac Studio reports 3840×2160 full repaint ≈0.67 FPS (avg 1.50 s) vs incremental strokes ≈6.5 FPS (avg 154 ms); 1280×720 full repaint ≈5.9 FPS (avg 170 ms) vs incremental ≈54 FPS (avg 18 ms).)
  - Parallelize the encode phase. Software encode currently dominates full-repaint frame time; split the raster/compose loop into per-tile jobs and feed them through the same per-core queue used by progressive copies so multi-core hosts reclaim throughput headroom.
- Runtime targets:
