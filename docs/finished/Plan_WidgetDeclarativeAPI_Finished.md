...
4. **Post-mortem (November 28, 2025)**
   ...
   - **Performance recap:** The `widget_pipeline` guardrail now measures the declarative path directly (mutations, descriptor synthesis, paint GPU uploads) and stabilised at ~1.05 ms per iteration after the schema/journal rewrite, up from 0.084 ms when builders owned the cache. Paint GPU uploads remained within the refreshed tolerances (`declarative.paintGpuLastUploadNs ≈ 1.1 ms`) and the screenshot harness stayed green once the readiness/pump helpers were introduced.
   ...
   - **Remaining TODOs:** (1) Continue exporting `PATHSPACE_LEGACY_WIDGET_BUILDERS=error` in CI/pre-push and monitor the diagnostics counters; delete the telemetry namespace once it stays at zero through the support-window deadline. (2) Rebaseline `docs/perf/performance_baseline.json` again if descriptor or paint-runtime changes move the widget-pipeline numbers materially; the guardrail is our regression alarm going forward. (3) Promote the screenshot fallback flag to onboarding docs if we decide to keep software captures as the default in CI.
