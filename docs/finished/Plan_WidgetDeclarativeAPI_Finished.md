...
- ✅ (November 19, 2025) `examples/widgets_example_minimal.cpp` moved to the declarative helper for a compact repro, and **refresh (November 28, 2025):** the sample now mirrors the plan doc quickstart—`SP::System::LaunchStandard` → `SP::App::Create` → `SP::Window::Create` → `SP::Scene::Create` → declarative `Label`/`Button`/`List` wiring—and hands off to `SP::App::RunUI` so contributors have a literal reference for the documented flow.
   - ✅ (November 19, 2025) Added `examples/declarative_hello_example.cpp` as the quick-start sample. **Update (November 28, 2025):** the sample now intentionally mounts only a single declarative button so the onboarding docs have a literal “hello world” artefact without extra layout noise; list + label coverage lives in `widgets_example_minimal.cpp`.
...
2. **Performance validation**
   - ✅ (November 25, 2025) Added the deterministic `benchmarks/ui/widget_pipeline_benchmark.cpp` harness, which replays identical button/toggle/slider/list/paint loads through the legacy builders and the declarative runtime and records bucket latency/bytes alongside declarative dirty-widget throughput and paint GPU upload timing for regression tracking.
...
2. **Support window**
   ...
   - ✅ (November 26, 2025) `scripts/compile.sh`, the local pre-push hook, and CI now export `PATHSPACE_LEGACY_WIDGET_BUILDERS=error` by default, stream every guard hit into the JSONL reporter specified via `PATHSPACE_LEGACY_WIDGET_BUILDERS_REPORT`, and (historically) relied on the shared test runner’s automatic `LegacyBuilders::ScopedAllow` handling for `tests/ui/*` so compatibility suites could run while guards were active. The guard has since been removed.
3. **Removal**
   - ✅ (November 28, 2025) Legacy builders have been fully retired. All public headers now live under `include/pathspace/ui/runtime/` (`UIRuntime.hpp`, `TextRuntime.hpp`, `SurfaceTypes.hpp`, `RenderSettings.hpp`), the source files were renamed to their `*Runtime.cpp` counterparts, and every include/namespace reference now targets `SP::UI::Runtime::*`. The legacy `Builders.hpp`, wrapper headers, `LegacyBuildersDeprecation`, and the kill-switch script/flags were deleted from the tree, so declarative/runtime code is the only UI entry point.
   - ✅ (November 28, 2025) Tooling, docs, and automation were updated to reference the runtime namespace. `docs/WidgetDeclarativeAPI.md`, onboarding guides, the migration tracker, and Memory now describe the runtime helpers; `scripts/git-hooks/pre-push.local.sh` no longer runs the kill-switch dry run, and CMake no longer exposes `PATHSPACE_DISABLE_LEGACY_BUILDERS`.
   - ✅ (November 28, 2025) Archived the last builder-era reference material (`docs/finished/Plan_WidgetDeclarativeAPI_EventRouting_Finished.md`) so only historical docs mention the shelved routing-table experiment.
4. **Post-mortem (November 28, 2025)**
   - **Outcome:** Declarative widgets replaced every `SP::UI::Builders::*` surface, samples/tests/docs now boot exclusively through `SP::UI::Declarative::*`, and the kill-switch guard plus telemetry proved no legacy code paths remain. The event-routing inventory was archived under `docs/finished/` and every contributor-facing guide points at the declarative runtime.
   - **Performance recap:** The `widget_pipeline_benchmark` now measures the declarative path directly (mutations, descriptor synthesis, paint GPU uploads) and stabilised at ~1.05 ms per iteration after the schema/journal rewrite, up from 0.084 ms when builders owned the cache. Paint GPU uploads remained around 1.1 ms and the screenshot harness stayed green once the readiness/pump helpers were introduced.
   - **Operational lessons:** `PATHSPACE_SCREENSHOT_FORCE_SOFTWARE=1` plus the screenshot CLI flags (`--force-software/--allow-software-fallback`) were critical for deterministic CI captures while Metal frame-grabs remain flaky on headless hosts. Manual pump hooks (`PumpWindowWidgetsOnce`, `SceneLifecycle::PumpSceneOnce`) need to stay in the toolbox for future trellis work; wiring them into `PathSpaceExamples::ensure_declarative_scene_ready` kept the mandated 5× loop under the 20 s timeout ceiling. Keep `/_system/diagnostics/legacy_widget_builders/*` publishing until the February 1, 2026 support window closes so downstream repos can still detect straggling binaries.
   - **Remaining TODOs:** (1) Continue exporting `PATHSPACE_LEGACY_WIDGET_BUILDERS=error` in CI/pre-push and monitor the diagnostics counters; delete the telemetry namespace once it stays at zero through the support-window deadline. (2) Capture fresh `widget_pipeline_benchmark` metrics whenever descriptor or paint-runtime changes move the numbers materially so dashboards keep reflecting the latest steady state. (3) Promote the screenshot fallback flag to onboarding docs if we decide to keep software captures as the default in CI.
