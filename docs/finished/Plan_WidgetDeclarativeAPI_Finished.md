...
1. **Examples**
   ...
   - ✅ (November 19, 2025) `examples/widgets_example_minimal.cpp` was rewritten on the same helper to provide a compact button+slider repro for declarative plumbing, and both demos reuse `Runtime::App::PresentToLocalWindow` for the LocalWindow bridge rather than the legacy reducer loop.
