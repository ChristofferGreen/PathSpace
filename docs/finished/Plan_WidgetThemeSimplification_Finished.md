...
- **Palette monitoring (Completed November 30, 2025):** `scripts/paint_example_monitor.py` now enforces both the manifest-revision lock file and a new ledger (`docs/images/paint_example_palette_log.md`). Every screenshot guardrail run validates that the manifest revision recorded in `docs/images/paint_example_baselines.json` has a matching ledger entry, and it fails immediately if any capture’s SHA-256 deviates from the manifest. Refresh baselines via `scripts/paint_example_capture.py`, bump the manifest + lock file together, append a ledger entry, and note the change in `docs/Memory.md`.
  - Baselines remain at revision 8 (November 30, 2025); future palette deltas must include the ledger update plus an explanation in the Memory log before landing.
