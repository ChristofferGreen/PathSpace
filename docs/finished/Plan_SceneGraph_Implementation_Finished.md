...
> **Completion (October 30, 2025):** Cleared the PathRenderer2D/pixel_noise performance guardrail regression by clipping rect draws to damage regions, skipping render passes when no damage is present, and persisting overdraw metrics between frames. Guardrail runs now report `path_renderer2d` incremental ≈1.2 ms (vs 7.57 ms baseline target) and `pixel_noise_software` 169 FPS / 5.9 ms present-call, unblocking pre-push checks.
...
> **Completion (October 29, 2025):** Integrated the resource-backed FontManager pipeline: widget demos register PathSpaceSans regular/semibold fonts, `TypographyStyle` records resource roots/revisions/fingerprints, and `TextBuilder` emits font fingerprints with doctests covering the new metadata paths and atlas bookkeeping.
> **Completion (October 29, 2025):** Font metadata now lives in PathSpace (`meta/{family,style,weight,fallbacks,active_revision}`); `FontManager::resolve_font` reads those nodes, surfaces active atlas revisions, and updates widget examples/tests to hydrate typography directly from PathSpace data with doctests covering deduplicated fallbacks and default handling.
> **Completion (October 29, 2025):** Landed `examples/widgets_example_minimal.cpp`, originally a lightweight slider/list/tree showcase with keyboard focus navigation and no diagnostics/screenshot plumbing so contributors could explore the GUI builder surface quickly. **Update (November 28, 2025):** the demo now mirrors the plan doc’s declarative quickstart (LaunchStandard → App/Window/Scene helpers → `Label` + `Button` + `List`, finished with `SP::App::RunUI`) so contributors see the documented flow verbatim in code.
...
- ✅ (October 21, 2025) Added a reusable stroke primitive (polyline/triangle strip) so paint examples track a point list per stroke instead of emitting per-dab rects; renderer + HTML adapter now consume the command and paint_example switched to the new primitive.
  - ✅ (October 21, 2025) Provide styling hooks (colors, corner radius, typography) so demos can skin widgets without forking scenes. `Widgets::WidgetTheme` centralises per-widget styles, builders publish the metadata, and `widgets_example` now loads a theme selectable via `WIDGETS_EXAMPLE_THEME` (default or `sunset`). Themes persist under `config/theme/<name>/value` so palette tweaks flow through PathSpace like other settings.
