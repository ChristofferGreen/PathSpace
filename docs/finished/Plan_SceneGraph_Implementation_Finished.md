...
> **Completion (October 30, 2025):** Cleared the PathRenderer2D/pixel_noise performance regression by clipping rect draws to damage regions, skipping render passes when no damage is present, and persisting overdraw metrics between frames. Benchmark runs now report `path_renderer2d` incremental ≈1.2 ms (vs 7.57 ms baseline target) and `pixel_noise_software` 169 FPS / 5.9 ms present-call, unblocking pre-push checks.
