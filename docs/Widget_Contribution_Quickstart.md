...
- `examples/widgets_example_minimal.cpp` now mirrors the plan doc sample directly: LaunchStandard → App::Create → Window::Create → Scene::Create → declarative `Label`/`Button`/`List`, then `SP::App::RunUI`. It avoids the shared helper on purpose so contributors can see the literal quickstart flow in code form.
- `examples/declarative_button_example.cpp` is the tiniest runnable snippet: it boots the runtime, mounts a single `Button`, and hands control to `SP::App::RunUI` in roughly 80 lines of code. Use it when you want to follow the exact syntax from the widget docs without any extra layout or handler scaffolding.
- `examples/declarative_hello_example.cpp` is now the literal quickstart: it creates an app, window, and a single declarative button, registers pointer/text devices, and runs the standard present loop. Point new contributors at this sample when they need the smallest runnable declarative app before layering in lists or composite layouts.
- Most declarative demos share the helper header. When you add a new sample (unless you intentionally follow the minimal example’s direct `App::RunUI` flow), include `"declarative_example_shared.hpp"`, call `SP::UI::Declarative::BuildPresentHandles` after `Window::Create`, register the pointer/text device you care about via `subscribe_window_devices`, and wrap the present loop with `run_present_loop`. Before presenting (or kicking off screenshot flows), call `PathSpaceExamples::ensure_declarative_scene_ready` so lifecycle metrics, scene structure, and the first snapshot are populated; this is now required for every helper-based demo so we never flash an empty window.
