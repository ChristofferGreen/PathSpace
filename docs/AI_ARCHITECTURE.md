...
> **Descriptor update (November 13, 2025 — evening):** Each trellis publishes a `TrellisRuntimeDescriptor` under `/_system/trellis/internal/state/<hash>/runtime/descriptor`, wiring config, ready queue, waiters, cursor, back-pressure limit, shutdown flag, and trace snapshot into a single inspection payload. Tooling should prefer the descriptor over stitching together individual mirrors ahead of the Phase D cleanup.
