...
- Reducer helpers (`SP::UI::Declarative::Reducers::*`) drain `widgets/<id>/ops/inbox/queue`, mutate widget state (usually by rewriting `state` or metadata), and optionally mirror actions to `ops/actions/inbox/queue` for downstream consumers. The legacy `SP::UI::Runtime::Widgets::Reducers::*` wrappers now simply route through the declarative helpers after running the guard, so new code should never include the builder headers just to drain widget ops.
...
- `SP::System::LaunchStandard` (`include/pathspace/ui/declarative/Runtime.hpp`) now performs end-to-end bootstrap for declarative UI runs:
  ...
  - Starts the `/system/widgets/runtime/input` pump unless `LaunchOptions::start_input_runtime` is disabled. The worker drains `widgets/<widget>/ops/inbox/queue`, publishes `WidgetAction`s back to `ops/actions/inbox/queue`, mirrors those actions into the canonical widget event queues (`widgets/<widget>/events/inbox/queue` plus per-event queues such as `events/press/queue`), and maintains metrics under `/system/widgets/runtime/input/metrics/*` (`widgets_processed_total`, `widgets_with_work_total`, `actions_published_total`, `events_enqueued_total`, `events_dropped_total`, `last_pump_ns`). Failures enqueue strings to `/system/widgets/runtime/input/log/errors/queue`.
...
- `PathSpaceJsonExporter` builds on the visitor API to serialize nodes (path, children, per-value entries, truncation flags). `PathSpaceBase::toJSON` wraps the exporter so tooling can dump subtrees without wiring converters manually.
- `tools/pathspace_dump_json.cpp` provides the supported CLI surface: `pathspace_dump_json --demo --root /demo --max-depth 3 --max-children 4 --max-queue-entries 2 --output demo.json` seeds the demo tree, enforces traversal limits, and writes deterministic JSON that matches the inspector schema. The `PathSpaceDumpJsonDemo` test guards the CLI by diffing the output against `tests/fixtures/pathspace_dump_json_demo.json`.
...
- Every renderer target registers a `diagnostics/cacheWatch` sentinel the first time Builders cache a `PathSurfaceSoftware`/`PathSurfaceMetal`. Removing the target (or tearing down its app root) deletes the sentinel, whose destructor evicts the corresponding cache entries so long-lived sessions and 5× loop tests do not retain stale framebuffers.
...
- The C++ helper (`SceneSnapshotBuilder`, added October 14, 2025) clamps drawable SoA integrity and now emits structured binary artifacts (`drawables.bin`, `transforms.bin`, `bounds.bin`, `state.bin`, `cmd-buffer.bin`, plus optional index files) under `scenes/<id>/builds/<revision>/bucket/`. A compact binary manifest stored at `drawable_bucket` records counts and layer ids for consumers. After writing the files it atomically publishes via `Runtime::Scene::PublishRevision`, maintains a rolling index at `meta/snapshots/index`, and enforces the default retention policy (≥3 revisions or ≥2 minutes) while preserving `current_revision`.
