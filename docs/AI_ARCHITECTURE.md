...
- `Scene::MarkDirty` updates `scenes/<scene-id>/diagnostics/dirty/state` with a monotonically increasing sequence, merged dirty mask, and timestamp, and pushes matching `Scene::DirtyEvent` entries onto `diagnostics/dirty/queue`. `Scene::TakeDirtyEvent` (blocking) and `Scene::ReadDirtyState` give renderers a wait/notify friendly surface for scheduling layout/rebuild work without polling.
...
- On macOS the example presenter (`examples/macos/WindowEventPump.mm`) now owns a `CAMetalLayer`, grabs `CAMetalDrawable` instances, copies the most recent software framebuffer directly into the drawable’s `IOSurface`, and presents via an empty Metal command buffer. This removes the CoreGraphics blit path for fullscreen windows; the CPU writes directly into the texture that will be scanned out, deferring only the GPU-present fence.
...
- Next milestone: propagate per-drawable damage so PathRenderer2D can avoid repainting the entire surface each frame (see docs/SceneGraphImplementationPlan.md “Incremental software renderer”); until then, fullscreen canvases still pay the cost of full-surface redraws.
