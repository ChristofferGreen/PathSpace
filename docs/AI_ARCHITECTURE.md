...
- Reducer helpers (`SP::UI::Declarative::Reducers::*`) drain `widgets/<id>/ops/inbox/queue`, mutate widget state (usually by rewriting `state` or metadata), and optionally mirror actions to `ops/actions/inbox/queue` for downstream consumers. The legacy `SP::UI::Builders::Widgets::Reducers::*` wrappers now simply route through the declarative helpers after running the guard, so new code should never include the builder headers just to drain widget ops.
