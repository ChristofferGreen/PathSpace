...
- `SP::System::LaunchStandard` (`include/pathspace/ui/declarative/Runtime.hpp`) now performs end-to-end bootstrap for declarative UI runs:
  ...
  - Starts the `/system/widgets/runtime/events` routing trellis unless `LaunchOptions::start_widget_event_trellis` is disabled. `CreateWidgetEventTrellis` drains the per-window event queues, performs `Scene::HitTest`, and emits `WidgetOp`s (hover/press/release/activate/toggle) for button/toggle widgets today, logging misses under `/system/widgets/runtime/events/log/errors/queue` and publishing telemetry to `/system/widgets/runtime/events/metrics/*` (`pointer_events_total`, `button_events_total`, `widget_ops_total`, `hit_test_failures_total`, `last_dispatch_ns`). The worker’s lifecycle flag lives at `/system/widgets/runtime/events/state/running`.
...
- `CreateInputTask` seeds `/system/widgets/runtime/input`:
  ...
  - After reducers publish `WidgetAction`s, the worker resolves each `events/<event>/handler = HandlerBinding`, looks up the registered lambda, invokes button/toggle/slider/list/tree/input handlers, and records any registry mismatches or exceptions back to the log + metrics roots.
...
- `LaunchOptions::start_widget_event_trellis` defaults to `true`. `widget_event_options` expose the routing worker’s refresh cadence, path roots (`windows_root`, `events_root`, `metrics_root`, `log_root`, `state_path`), and an optional `hit_test_override` hook for tests. `CreateWidgetEventTrellis` keeps `/system/widgets/runtime/events/state/running` updated, mirrors metrics to `/system/widgets/runtime/events/metrics/{pointer_events_total,button_events_total,widget_ops_total,hit_test_failures_total,last_dispatch_ns}`, and logs failures to `/system/widgets/runtime/events/log/errors/queue`. The current implementation emits `WidgetOp`s for button/toggle widgets (hover/press/release/activate/toggle); slider/list/tree/text routing still needs to land before those handler paths receive live events.
