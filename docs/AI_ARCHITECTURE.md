...
> **Concurrency update (November 13, 2025):** Trellis waiters, round-robin cursor updates, ready queues, shutdown flags, traces, and latest snapshot payloads now write exclusively through `TrellisRuntimeDescriptor` under `/_system/trellis/internal/state/<hash>/runtime/descriptor`. Compatibility mirrors (`ready/{queue,count}`, `waiters`, `cursor`, `flags/shutting_down`, `trace/latest`) are refreshed by descriptor writes, and `PATHSPACE_TRELLIS_INTERNAL_RUNTIME=0` now emits a warning while keeping the mirrors enabled—rollback is handled by reverting the descriptor change, not by disabling it.
> **Concurrency update (November 14, 2025):** Queue-mode coverage now asserts descriptor-backed traces end-to-end via the new `PathSpaceTrellis::debugRuntimeDescriptor` probe. Internal trace mirrors and persisted stats are exercised in the regression suite, so you can rely on the descriptor snapshot for live tooling instead of stitching together legacy mutex state. A follow-up “Phase E” task in `Plan_PathSpace_FanIn.md` tracks the remaining work to move enable/disable/config mutations into internal `PathSpace` transactions and retire the last `TrellisState` mutex once descriptor-backed config updates are in place.
> **Descriptor update (November 13, 2025 — evening):** `Runtime descriptor mirrors trellis state` covers the descriptor fields end-to-end so tooling can rely on a single inspection payload instead of piecing together mutex-backed state.
> **Fan-in roadmap (November 14, 2025):** `Plan_PathSpace_FanIn.md` now captures a descriptor-first buffered fan-in roadmap (BF-1 → BF-4) that introduces per-source buffering metadata, replay cursors, and mirrored inspector paths. Use the roadmap when planning any trellis changes that read/write buffered depth so schema updates stay aligned with the runtime descriptor.
