...
> **Diagnostics update (October 17, 2025):** Per-frame timing metrics (`damageMs`, `encodeMs`, `progressiveCopyMs`, `publishMs`, `presentMs`) are emitted alongside `renderMs` so benchmarks and demos (`./build/paint_example --debug`) can attribute performance costs across the pipeline.
