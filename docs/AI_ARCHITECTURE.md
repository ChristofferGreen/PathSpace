...
- `PathSpaceJsonExporter` builds on the visitor API to serialize nodes (path, children, per-value entries, truncation flags). `PathSpaceBase::toJSON`/`writeJSONToFile` wrap the exporter so tooling can dump subtrees without wiring converters manually.
