...
- `VisitOptions::UnlimitedDepth`/`kUnlimitedChildren` document the sentinel values, `childLimitEnabled()` reports whether the cap is active, and the exporter/CLI surface `"max_children": "unlimited"` whenever the child limit is disabled (still bounded by `maxDepth`).
