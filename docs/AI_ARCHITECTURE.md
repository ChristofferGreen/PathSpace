...
- November 7, 2025: Execution payloads (tasks/futures) stay outside undo snapshots. Callers register undo-exempt branches with `HistoryOptions::executionOptOutPrefixes`, mutations under those prefixes skip snapshot capture, and `_history/executionOptOut/*` telemetry tracks the paths/occurrence counts so integrators can confirm guardrails. `_history/unsupported/*` continues to log nested PathSpaces and serialization failures.
