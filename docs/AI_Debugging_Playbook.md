...
- **Trellis runtime mirrors (November 13, 2025):** Internal fan-in bookkeeping now dual-writes into `/_system/trellis/internal/state/<hash>/runtime/` (`ready/{queue,count}`, `waiters`, `cursor`, `flags/shutting_down`, `trace/latest`, and the consolidated `descriptor`). Use `PATHSPACE_TRELLIS_INTERNAL_RUNTIME=0` to disable the mirrors when isolating regressions. See `tests/unit/layer/test_PathSpaceTrellis.cpp` (“Internal runtime mirrors ready queue state”, “Internal runtime tracks waiters, cursor, and shutdown”, “Runtime descriptor mirrors trellis state”) for concrete readback examples and expected data shapes.
  ...
  - **Loop perf harness (November 13, 2025):** `./scripts/compile.sh --test --loop=15 --release --runtime-flag-report` runs the mandated loop twice (`PATHSPACE_TRELLIS_INTERNAL_RUNTIME=1` then `0`) and writes `build/trellis_flag_bench.json` with total/average timings. Latest run (November 13, 2025 @ 18:28 UTC) completed all 15 iterations in both modes; the JSON lists the aggregate runtimes so you can diff flag-on/off overhead without scraping logs.
