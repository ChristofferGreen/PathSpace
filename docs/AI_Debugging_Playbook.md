...
- **Trellis runtime mirrors (November 13, 2025):** Internal fan-in bookkeeping now dual-writes into `/_system/trellis/internal/state/<hash>/runtime/` (`ready/{queue,count}`, `waiters`, `cursor`, `flags/shutting_down`, `trace/latest`). Use `PATHSPACE_TRELLIS_INTERNAL_RUNTIME=0` to disable the mirrors when isolating regressions. See `tests/unit/layer/test_PathSpaceTrellis.cpp` (“Internal runtime mirrors ready queue state”, “Internal runtime tracks waiters, cursor, and shutdown”) for concrete readback examples and expected data shapes.
