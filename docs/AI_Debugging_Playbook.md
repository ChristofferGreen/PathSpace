...
- **Visitor snapshots (November 30, 2025):** `PathSpaceBase::visit(PathVisitor const&, VisitOptions const&)` replaces ad-hoc `listChildren` crawls. Set `VisitOptions` (`root`, `maxDepth`, `maxChildren`, `includeNestedSpaces`, `includeValues`) and supply a callback that receives `PathEntry` metadata plus a `ValueHandle`. Use the handle’s `read<T>()` only when `includeValues=true`; otherwise call `snapshot()` to inspect queue metadata without copying bytes. Layers (PathView/Alias/Trellis/UndoableSpace) already delegate to their upstreams, so the same visitor works on aliased spaces, inspector views, or trellis mounts. Set `maxChildren = VisitOptions::UnlimitedChildren` (0) whenever you need every child; the exporter/CLI will report `"max_children": "unlimited"` so diagnostics know the cap is disabled.
- **JSON dumps (December 28, 2025):** Exporter stays unfiltered and `_meta` is opt-in (`--include-meta` or `PathSpaceJsonOptions::includeMetadata`); debug mode still auto-enables structure fields/diagnostics/placeholders/metadata. Defaults are now unbounded: `max_depth=unlimited`, `max_children=unlimited`, `max_queue_entries=unlimited`, nested spaces off, values on, diagnostics off, indent=2. `./build/declarative_button_example --dump_json` relies on those defaults (rooted at the app) so widget labels render without truncation; `--dump_json_debug` flips on the debug preset. `pathspace_dump_json` mirrors the same defaults and exposes explicit caps via `--max-depth/--max-children/--max-queue-entries` when you need pruning. Custom type converters still register via `PathSpaceJsonRegisterConverterAs<T>("FriendlyType", lambda)` so `values[*].type` stays readable.
...
- When capturing paint-example baselines manually, set `PATHSPACE_SCREENSHOT_FORCE_SOFTWARE=1` (or run `pathspace_screenshot_cli --force-software`) if `Window::Present` refuses to capture on headless Metal hosts. The CLI and Python helper both read `hardware_capture` from the metrics JSON and exit non-zero if a fallback happens without the guard, which keeps captures honest while still giving headless hosts a deterministic path.
...
1. Dump the `/renderers` subtree (enable diagnostics with `PATHSPACE_UI_DEBUG_TREE=1` first):
...
- **Dashboards & thresholds:** Residency and progressive feeds are wired into the shared dashboards so on-call engineers do not need to reverse engineer path names when paging:
...
- `python3 scripts/check_pixel_noise_baseline.py --build-dir build` can still be run manually to compare the pixel-noise example against the recorded baselines, but the old `PixelNoisePerfHarness*` CTest entries were removed from the automated loop until the ServeHtml/example rewrite provides a faster harness.
