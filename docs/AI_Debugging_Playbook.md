...
- **Trellis runtime mirrors (November 14, 2025):** Internal fan-in bookkeeping still dual-writes into `/_system/trellis/internal/state/<hash>/runtime/` (`ready/{queue,count}`, `waiters`, `cursor`, `flags/shutting_down`, `trace/latest`, and the consolidated `descriptor`). `PATHSPACE_TRELLIS_INTERNAL_RUNTIME=0` now logs a rollback warning but keeps the mirrors enabled; rely on regressions + `git revert` for rollback instead of toggling the flag. Use `PathSpaceTrellis::debugRuntimeDescriptor(path)` in tests or diagnostics when you need the in-memory descriptor snapshot without touching persistence; production tooling should read the same payload via the internal space. See `tests/unit/layer/test_PathSpaceTrellis.cpp` (“Queue mode blocks until data arrives”, “Internal runtime mirrors ready queue state”, “Internal runtime tracks waiters, cursor, and shutdown”, “Runtime descriptor mirrors trellis state”) for concrete readback examples and expected data shapes. `Plan_PathSpace_FanIn.md` now documents a descriptor-first buffered fan-in roadmap: expect new `descriptor.bufferedSources[*]` metadata once BF-1 lands and update troubleshooting scripts to report per-source depth counters when available. Phase E will migrate the remaining enable/disable/config mutations into the embedded `PathSpace` so the descriptor can own multithreaded coordination without the legacy mutex.
  - **Serialization note (November 13, 2025):** Waiter snapshots/flags read back via bespoke helpers in `type/serialization.hpp` (no Alpaca). If doctests crash or the loop reports malformed buffers, inspect those helpers first and confirm the internal runtime nodes match the expected binary layout.
  - **Loop perf harness (November 13, 2025):** `./scripts/compile.sh --test --loop=15 --release --runtime-flag-report` still runs the mandated loop twice (`PATHSPACE_TRELLIS_INTERNAL_RUNTIME=1` then `0`) and writes `build/trellis_flag_bench.json`. The `{1,0}` run records comparative timing data even though the `0` run leaves the mirrors active.
