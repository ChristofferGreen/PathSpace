...
- **JSON dumps (December 27, 2025):** Exporter no longer filters paths or hides “noisy” branches—the tree itself is already minimal. `_meta` is now opt-in (`--include-meta` or `PathSpaceJsonOptions::includeMetadata`); debug mode auto-enables it alongside structure fields/diagnostics/placeholders. `./build/declarative_button_example --dump_json` emits the full (still lean) app tree with a `max_depth` of 10 so widget labels stay visible; scene snapshots keep only the latest revision when debug flags are off, so the dump stays ~1k lines without filters. Add `--dump_json_debug` for the verbose view. `pathspace_dump_json` defaults to minimal (no diagnostics/placeholders/metadata, nested spaces off, `max_depth=6`, `max_children=64`, `max_queue_entries=2`, `indent=2`) and exposes `--debug` for the detailed variant. Custom type converters still register via `PathSpaceJsonRegisterConverterAs<T>("FriendlyType", lambda)` so `values[*].type` stays readable.
