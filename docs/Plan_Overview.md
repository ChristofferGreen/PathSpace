...
- ✅ (November 25, 2025) Legacy builder deprecation telemetry landed: every legacy `SP::UI::Builders::*` entry now records usage under `/_system/diagnostics/legacy_widget_builders/<entry>/*` and the runtime honors `PATHSPACE_LEGACY_WIDGET_BUILDERS={allow,warn,error}` (default `warn`) so CI and pre-push hooks can flip to hard failures ahead of the February 1, 2026 support-window cutoff.
