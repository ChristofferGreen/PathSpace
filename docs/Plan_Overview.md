...
- ✅ (November 28, 2025) Legacy builder runtime removed: the `SP::UI::Runtime` headers and `*Runtime.cpp` sources now own the UI helper surface, `LegacyBuildersDeprecation` + kill-switch guards were deleted, and CI/pre-push rely solely on the declarative/runtime pipeline. Only the Phase 4 post-mortem from `Plan_WidgetDeclarativeAPI` remains.
