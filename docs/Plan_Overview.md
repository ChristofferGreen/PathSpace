...
## Status Snapshot — November 6, 2025
- ✅ (November 6, 2025) UndoableSpace persistence and unsupported-payload coverage landed: snapshots/meta write atomically, manual GC flushes pending writes, disk telemetry reflects on-disk history, restart recovery replays into the live PathSpace, and regression tests now confirm tasks/futures or nested PathSpaces emit descriptive errors. Inspector telemetry now exposes the offending path/reason via `<root>/_history/unsupported/*`; `docs/Plan_PathSpace_UndoHistory.md` tracks the remaining decision on executable payload snapshot support (Step 5).
