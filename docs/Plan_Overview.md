...
- Pixel noise perf harness (`examples/pixel_noise_example.cpp`) now drives per-pixel full-surface churn with the software renderer; enable `--report-metrics`/`--report-extended` when you need periodic FPS diagnostics. Automation/baseline follow-ups remain tracked in `Plan_SceneGraph_Implementation.md`.
