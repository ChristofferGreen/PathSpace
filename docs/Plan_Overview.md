...
- ✅ (November 7, 2025) UndoableSpace persistence and unsupported-payload coverage remain solid, and the persistence-format bake-off is resolved: entry/state metadata now emits compact versioned binary headers, recovery paths share a single codec, and the undo tests cover encode/decode round-trips. `docs/Plan_PathSpace_UndoHistory.md` and downstream plans now reflect the format choice and call out the remaining integration/documentation tasks.
