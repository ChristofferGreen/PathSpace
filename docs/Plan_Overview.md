...
- ✅ (November 7, 2025) UndoableSpace persistence and unsupported-payload coverage remain solid, and the persistence-format bake-off is resolved: entry/state metadata now emits compact versioned binary headers, recovery paths share a single codec, and the undo tests cover encode/decode round-trips. `docs/Plan_PathSpace_UndoHistory.md`, `docs/Plan_PathSpace.md`, the inspector, and paint widget plans now reference the binary codec plus `_history/stats/*` telemetry, with follow-on work focusing on CLI/debugging helpers for inspecting on-disk history.
