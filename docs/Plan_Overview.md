...
- Pixel noise perf harness (`examples/pixel_noise_example.cpp`) now drives per-pixel full-surface churn with the software renderer; runs headless by default for uncapped FPS and supports `--windowed --present-refresh=<hz>` plus `--report-metrics`/`--report-extended` when you need on-screen diagnostics. Automation/baseline follow-ups remain tracked in `Plan_SceneGraph_Implementation.md`.
