...
- ➡️ Next focus: after the declarative UI widget rewrite lands, promote the paint example (and downstream paint widget) to mount its buffer under an `UndoableSpace`, wiring `_history/undo` / `_history/redo` so we exercise interactive history flows end-to-end.
