...
| Telemetry & diagnostics | Legacy widgets emitted limited per-widget metrics. | Declarative runtime publishes schema/focus/input/render telemetry plus per-widget logs (`docs/finished/Plan_WidgetDeclarativeAPI_Finished.md:242-272`, `docs/finished/Plan_WidgetDeclarativeAPI_EventRouting_Finished.md:5-62`). | ✅ Broader coverage (handlers, focus, lifecycle, GPU uploads) and shared logging roots. |
...
The matrix above should be refreshed whenever new widgets land or when telemetry/worker contracts change. Update this file and the Phase 3 checklist inside `docs/finished/Plan_WidgetDeclarativeAPI_Finished.md` together to keep planning artifacts in sync.
