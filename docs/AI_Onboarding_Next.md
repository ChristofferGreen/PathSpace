...
- Declarative screenshots are standardized: `widgets_example`, `declarative_hello_example`, and `devices_example --paint-controls-demo` all share the same headless CLI (`--screenshot`, optional `--screenshot-compare/--screenshot-diff/--screenshot-metrics`, force-software toggles) layered on top of `CaptureDeclarative`. Use whichever sample best exercises the UI you are touching.
...
- Latest change: the declarative demos (`widgets_example`, `declarative_hello_example`, and `devices_example --paint-controls-demo`) now share the screenshot CLI layered on `CaptureDeclarative`, so any of them can render a headless PNG (with optional compare/diff/metrics artifacts) without touching bespoke boilerplate.
