...
5. **Quick Test Pass (recommended)**
  ...
  - **Visual sanity check:** the declarative demos now share the same screenshot CLI from `<pathspace/ui/screenshot/DeclarativeScreenshotCli.hpp>`. Run `./build/widgets_example --screenshot /tmp/widgets_gallery.png [--screenshot-compare <baseline> --screenshot-diff <diff> --screenshot-metrics <metrics.json>]` for the gallery, `./build/declarative_hello_example --screenshot /tmp/hello.png` for the minimal sample, or `./build/devices_example --paint-controls-demo --screenshot /tmp/devices_controls.png` to snapshot the paint-controls UI. Each command runs headless, performs a deterministic pose, and exits once `CaptureDeclarative` finishes. Pair these with `./build/paint_example --screenshot /tmp/paint_demo.png` when you need the paint baseline replay. All declarative demos still call `PathSpaceExamples::ensure_declarative_scene_ready` before presenting, so you can assume lifecycle metrics + scene structure are fully populated before these screenshots fire.
...
- Declarative screenshots are standardized in `<pathspace/ui/screenshot/DeclarativeScreenshotCli.hpp>`: `widgets_example`, `declarative_hello_example`, and `devices_example --paint-controls-demo` all share the same headless CLI (`--screenshot`, optional `--screenshot-compare/--screenshot-diff/--screenshot-metrics`, force-software toggles) layered on top of `CaptureDeclarative`. Use whichever sample best exercises the UI you are touching.
...
- Latest change: the declarative demos (`widgets_example`, `declarative_hello_example`, and `devices_example --paint-controls-demo`) now share the screenshot CLI implemented in `<pathspace/ui/screenshot/DeclarativeScreenshotCli.hpp>`, so any of them can render a headless PNG (with optional compare/diff/metrics artifacts) without touching bespoke boilerplate.
