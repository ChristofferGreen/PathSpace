...
2. **Read-before-you-touch**
   ...
   - **Deprecation telemetry:** Every legacy builder entry reports to `/_system/diagnostics/legacy_widget_builders/<entry>/`. Keep the counters at zero and investigate any spike; those values now only change if an older binary or downstream consumer still references the removed API.
...
5. **Quick Test Pass (recommended)**
  ...
  - **Visual sanity check:** the declarative demos that ship screenshots now share the same CLI from `<pathspace/ui/screenshot/DeclarativeScreenshotCli.hpp>`. Run `./build/widgets_example --screenshot /tmp/widgets_gallery.png [--screenshot-compare <baseline> --screenshot-diff <diff> --screenshot-metrics <metrics.json>]` for the gallery, or `./build/devices_example --paint-controls-demo --screenshot /tmp/devices_controls.png` to snapshot the paint-controls UI. Each command runs headless, performs a deterministic pose, and exits once `CaptureDeclarative` finishes. Pair these with `./build/paint_example --screenshot /tmp/paint_demo.png` when you need the paint baseline replay. `examples/declarative_hello_example` remains CLI-free for the literal quickstart (it now only mounts a single declarative button), but exporting `PATHSPACE_HELLO_SCREENSHOT=/tmp/hello.png` (plus `PATHSPACE_HELLO_SCREENSHOT_FORCE_SOFTWARE=1` when Metal capture is unavailable) triggers the same helper after readiness completes and falls back to a deterministic reference render if the presenter never yields a framebuffer.
...
- List widget builder (`Runtime::Widgets::CreateList`) plus `UpdateListState` and `DispatchList` land with doctest coverage, enabling selection/hover/scroll ops and expanding `widgets_example`.
...
- `Runtime::App::Bootstrap` wires a renderer/surface/window + present policy for a scene in one call, trimming boilerplate in examples/tests (October 21, 2025).
- `Runtime::App::UpdateSurfaceSize` and `Runtime::App::PresentToLocalWindow` now own LocalWindow resize/present scaffolding; widgets_example, pixel_noise_example, and paint_example consume the helpers instead of bespoke loops (October 23, 2025).
...
- Latest change: the declarative demos that need automation (`widgets_example`, `devices_example --paint-controls-demo`) now share the screenshot CLI implemented in `<pathspace/ui/screenshot/DeclarativeScreenshotCli.hpp>`, so they can render headless PNGs (with optional compare/diff/metrics artifacts) without bespoke boilerplate. `declarative_hello_example` continues to serve as the smallest interactive quickstart sample while offering the env-driven screenshot hook (`PATHSPACE_HELLO_SCREENSHOT`) plus a deterministic software fallback when `Window::Present` never produces a framebuffer.
