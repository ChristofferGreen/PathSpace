...
5. **Quick Test Pass (recommended)**
  ...
  - **Visual sanity check:** the declarative demos that ship screenshots now share the same CLI from `<pathspace/ui/screenshot/DeclarativeScreenshotCli.hpp>`. Run `./build/widgets_example --screenshot /tmp/widgets_gallery.png [--screenshot-compare <baseline> --screenshot-diff <diff> --screenshot-metrics <metrics.json>]` for the gallery, or `./build/devices_example --paint-controls-demo --screenshot /tmp/devices_controls.png` to snapshot the paint-controls UI. Each command runs headless, performs a deterministic pose, and exits once `CaptureDeclarative` finishes. Pair these with `./build/paint_example --screenshot /tmp/paint_demo.png` when you need the paint baseline replay. `examples/declarative_hello_example` intentionally stays CLI-free as the literal quickstart but still calls `PathSpaceExamples::ensure_declarative_scene_ready` before presenting, so lifecycle metrics + scene structure are populated before the interactive loop starts.
