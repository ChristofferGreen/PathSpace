...
- ✅ (October 21, 2025) Shipped `examples/pixel_noise_example.cpp`, a per-pixel noise perf harness that uses the software renderer plus a before-present hook to generate full-surface churn each frame; it runs headless by default for uncapped FPS, with `--windowed` + `--present-refresh=<hz>` available when you want to mirror the compute stream to a local window (use `--report-metrics`/`--report-extended` to print diagnostics).
- ✅ (October 22, 2025) Pixel noise harness now defaults to the fast path (framebuffer capture off, metrics muted, headless mode) with CLI escapes for diagnostics/capture (`--capture-framebuffer`, `--report-metrics`, `--report-extended`, `--present-call-metric`, `--present-refresh=<hz>`).
