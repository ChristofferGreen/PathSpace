...
- 📌 (November 7, 2025) Wrap the paint example canvas/root in an `UndoableSpace`, persist stroke history into the undo layer, and surface explicit Undo/Redo affordances (buttons + shortcuts) that drive `_history/undo` / `_history/redo` so the demo exercises history integration end-to-end.
