...
- ✅ (October 23, 2025) Extracted widget helper internals into `WidgetBuildersDetail.inl` and split the widget runtime across `WidgetBuildersCore.cpp`, `WidgetBindings.cpp`, `WidgetFocus.cpp`, and `WidgetReducers.cpp`, keeping each unit <1 000 lines while preserving shared helpers via the new inline include.
