...
- ✅ (October 21, 2025) Shipped `examples/pixel_noise_example.cpp`, a per-pixel noise perf harness that uses the software renderer plus a before-present hook to generate full-surface churn each frame, and supports headless or windowed runs for quick throughput checks (use `--report-metrics`/`--report-extended` to print diagnostics).
- ✅ (October 22, 2025) Pixel noise harness now defaults to the fast path (framebuffer capture off, metrics muted) with CLI escapes for diagnostics/capture (`--capture-framebuffer`, `--report-metrics`, `--report-extended`, `--present-call-metric`).
