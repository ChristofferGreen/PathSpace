...
> **Handoff note (October 28, 2025 @ shutdown):** Landed canonical `Widgets::Build{Button,Toggle,Slider}Preview` helpers so demos/tests no longer hand-roll preview buckets, reset the default widget theme to the original blue palette to keep the HTML harness baselines green, and kept the shared `Widgets::Input` routing wired through widgets_example.
