...
4. 🚧 **Pre-push loop instability** – `./scripts/compile.sh --clean --test --loop=15 --release` still flakes inside `PathSpaceUITests`, but the harness now keeps every iteration’s full doctest log (`build/test-logs/loop_manifest.tsv` lists the saved files), exposes `--loop-keep-logs/--loop-label`, and lets us pass UI-only args such as `--ui-test-extra-args="--success"`. The flake itself still needs root-cause work, yet we finally have deterministic artifacts per loop without digging through truncated stdout.
