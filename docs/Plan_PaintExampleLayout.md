...
## Outstanding Issues
1. ⏳ **Screenshot drift** – Every visual change forces a new baseline PNG. Keep following the documented capture loop and track checksum/date updates inside this plan so review history stays obvious.
2. ⏳ **Pre-push loop follow-up** – Keep monitoring `PathSpaceUITests` after today’s fix; the timeout was rooted in GC trimming `/builds/*` faster than our test could observe it. If a new loop failure appears, capture the manifest/log pair before re-running the suite.
...
5. 🚧 Run `./scripts/compile.sh --clean --test --loop=15 --release` (and bump `--per-test-timeout 40` if a new failure needs more headroom). Archive `loop_run.log` plus any offending `build/test-logs/PathSpaceUITests_loop*.log`/artifacts whenever a loop iteration fails so we can diff against the November 21, 2025 green baseline (`PathSpaceUITests_loop15of15_20251121-113607.log`). This loop now executes both `PaintExampleScreenshot` and `PaintExampleScreenshot720`.
...
## Next Steps
1. ⏳ Surface the new history metrics inside the PathSpace inspector/debugging dashboards (plan callouts + UI wiring) so engineers can spot stuck bindings without opening the screenshot artifacts.
