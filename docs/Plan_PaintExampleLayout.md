...
## Next Steps
1. ⏳ Stabilize the compile-loop screenshot mismatches (`PaintExampleScreenshot*` still flakes under `./scripts/compile.sh --clean --test --loop=15 --release`—see `test-logs/loop_failures/20251122-134354_PaintExampleScreenshot_loop1/`). With force-publish + multi-process retries in place, the remaining failures consistently point to the controls/canvas drop-shadow strip; next step is to pin down the rendering ordering issue so the CLI stops needing retries entirely.
