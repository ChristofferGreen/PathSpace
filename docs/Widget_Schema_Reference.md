...
**Startup guard (Nov 20, 2025):** Declarative paint demos must wait for the lifecycle to finish publishing widget buckets before presenting or running the screenshot loop. `examples/paint_example.cpp` exposes `wait_for_widget_buckets` (polls `/runtime/lifecycle/metrics/widgets_with_buckets`) and `wait_for_scene_widgets` (watches `/scenes/<scene>/structure/widgets/windows/<window>/views/<view>/widgets`). Call both helpers after mounting widgets so users never see the empty window that appears while `SceneLifecycle` is still warming up.
