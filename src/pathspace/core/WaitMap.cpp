...
auto WaitMap::notify(GlobPathStringView const& path) -> void {
    sp_log("notify(glob): " + std::string(path.getPath()), "WaitMap");
    ...
    for (auto& [registeredPath, cv] : cvMap) {
        sp_log("Checking registered path: " + std::string(registeredPath.getPath()), "WaitMap");
        ...
        sp_log("Match result: " + std::to_string(isCompleteMatch), "WaitMap");
