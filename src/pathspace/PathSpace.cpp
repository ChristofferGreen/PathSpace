...
auto PathSpace::inInternal(ConstructiblePath& path,
...
auto PathSpace::inFinalComponent(ConstructiblePath& path,
...
auto PathSpace::inIntermediateComponent(ConstructiblePath& path,
                                        GlobPathIteratorStringView const& iter,
                                        GlobPathIteratorStringView const& end,
                                        GlobName const& pathComponent,
                                        InputData const& inputData,
...
auto PathSpace::inConcreteDataName(ConstructiblePath& path,
...
auto PathSpace::inGlobDataName(ConstructiblePath& path,
...
auto PathSpace::inGlobPathComponent(ConstructiblePath& path,
                                    GlobPathIteratorStringView const& iter,
                                    GlobPathIteratorStringView const& end,
                                    GlobName const& globName,
                                    InputData const& inputData,
    ...
    for (auto const& val : this->nodeDataMap) {
        if (std::get<0>(globName.match(val.first))) {
            if (std::holds_alternative<std::unique_ptr<PathSpace>>(val.second)) {
                ...
                std::get<std::unique_ptr<PathSpace>>(val.second)->inInternal(path2, std::next(iter), end, inputData, options, ret);
            }
        }
    }
}

auto PathSpace::inConcretePathComponent(ConstructiblePath& path,
                                        GlobPathIteratorStringView const& iter,
                                        GlobPathIteratorStringView const& end,
                                        ConcreteName const& concreteName,
                                        InputData const& inputData,
    ...
    auto const appendDataIfNameExists = [&](auto& nodePair) {
        if (std::holds_alternative<std::unique_ptr<PathSpace>>(nodePair.second))
            std::get<std::unique_ptr<PathSpace>>(nodePair.second)->inInternal(path, nextIter, end, inputData, options, ret);
        else
...
auto PathSpace::outInternal(ConcretePathIteratorStringView const& iter,
...
auto PathSpace::outDataName(ConcreteName const& concreteName,
...
auto PathSpace::outConcretePathComponent(ConcretePathIteratorStringView const& nextIter,
                                         ConcretePathIteratorStringView const& end,
                                         ConcreteName const& concreteName,
                                         InputMetadata const& inputMetadata,
                                         void* obj,
