...
auto make_paths(SP::App::AppRootPathView app_root,
    ...
    auto root = SP::App::resolve_app_relative(app_root, relative);
    if (!root) {
    ...
    ThemePaths paths{};
    paths.root = *root;
    paths.value = SP::App::ConcretePath{root->getPath() + "/value"};
    return paths;
}

auto active_theme_path(SP::App::AppRootPathView app_root) -> SP::Expected<SP::App::ConcretePath> {
    auto path = SP::App::resolve_app_relative(app_root, "config/theme/active");
    if (!path) {
...
auto Resolve(SP::App::AppRootPathView app_root,
    ...
    return make_paths(app_root, sanitized);
}
...
auto SetActive(PathSpace& space,
    ...
    auto sanitized = SanitizeName(theme_name);
    auto active = active_theme_path(app_root);
    if (!active) {
    ...
    SP::UI::Declarative::SceneLifecycle::InvalidateThemes(space, app_root);
    return {};
}
