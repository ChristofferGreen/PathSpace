...
auto prepare_surface_render_context(PathSpace& space,
    ...
    auto builder_context = SP::UI::Builders::Detail::prepare_surface_render_context(space,
                                                                                   surface,
                                                                                   settings_override);
    if (!builder_context) {
...
auto render_into_target(PathSpace& space,
    ...
    SP::UI::Builders::Detail::SurfaceRenderContext builder_context{
    ...
    return SP::UI::Builders::Detail::render_into_target(space,
                                                        builder_context,
                                                        surface
#if PATHSPACE_UI_METAL
                                                        ,
...
void invoke_before_present_hook(SP::UI::PathSurfaceSoftware& surface,
    ...
    SP::UI::Builders::Detail::invoke_before_present_hook(surface, policy, dirty_tiles);
}

auto renderer_kind_to_string(RendererKind kind) -> std::string {
    return SP::UI::Builders::Detail::renderer_kind_to_string(kind);
}
...
auto maybe_schedule_auto_render(PathSpace& space,
    ...
    -> SP::Expected<bool> {
    return SP::UI::Builders::maybe_schedule_auto_render(space, target_path, stats, policy);
}
...
auto write_present_metrics(PathSpace& space,
    ...
    return SP::UI::Builders::Diagnostics::WritePresentMetrics(space, target_path, stats, policy);
}
...
auto write_window_present_metrics(PathSpace& space,
                                  ...
                                  SP::UI::PathWindowPresentStats const& stats,
                                  SP::UI::PathWindowPresentPolicy const& policy) -> SP::Expected<void> {
    return SP::UI::Builders::Diagnostics::WriteWindowPresentMetrics(space, window_path, view_name, stats, policy);
}
...
auto write_residency_metrics(PathSpace& space,
    ...
    return SP::UI::Builders::Diagnostics::WriteResidencyMetrics(space,
                                                               target_path,
                                                               cpu_bytes,
                                                               gpu_bytes,
                                                               cpu_soft_bytes,
...
auto submit_dirty_rects(PathSpace& space,
    ...
    return SP::UI::Builders::Renderer::SubmitDirtyRects(space, target_path, rects);
}
