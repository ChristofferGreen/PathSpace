...
> **Test coverage (November 30, 2025):** `tests/ui/test_DeclarativeTheme.cpp` now exercises slider/toggle descriptor cases, tree palette overrides (including mask-bit assertions), and InputField/TextArea layout preservation so every widget with palette or structural overrides has regression tests guarding the new contract.
...
3. Updated buckets stay inside the lifecycle worker’s cache and are published directly into `RendererSnapshotStore` by `SceneSnapshotBuilder`. Presenter targets consume the in-memory snapshots; PathSpace does not mirror `render/bucket` or `/builds/<revision>/bucket/*` binaries anymore (only `current_revision` metadata persists).
...
- `render/synthesize` entries store a `RenderDescriptor` (widget kind enum). The runtime converts descriptors + `state/*` + `style/*` into a `WidgetDescriptor` (`Descriptor.hpp`) and synthesizes `DrawableBucketSnapshot` data via the legacy preview builders. Cached buckets remain in-memory within the lifecycle worker; `SceneSnapshotBuilder` publishes them directly into `RendererSnapshotStore`.
...
2. The scene lifecycle trellis (in-memory) drains dirty queues, calls into the descriptor synthesizer, caches updated buckets, and aggregates those buckets into scene-wide revisions via `SceneSnapshotBuilder` to `RendererSnapshotStore`. Snapshot stats mirror under `runtime/lifecycle/metrics` so tooling can inspect `widgets_registered_total`, `sources_active_total`, `events_processed_total`, `last_revision`, `last_published_widget`, `last_published_ms`, and `pending_publish` when throttling kicks in (metrics gated by `PATHSPACE_UI_DEBUG_TREE`).
...
4. Removal is logical (`state/removed = true`), but lifecycle workers immediately deregister trellis sources and evict cached buckets so metrics reflect the shrinking tree; no `scene/structure/widgets/.../render/bucket` mirrors remain.
